...
{  	 
	...
	RCC_AHB1PeriphClockCmd(USART2_GPIO_CLK|RCC_RS485_TXEN,ENABLE); //使能GPIOA时钟
	RCC_APB1PeriphClockCmd(USART2_CLK,ENABLE);//使能USART2时钟
	
  //串口2引脚复用映射
	GPIO_PinAFConfig(USART2_GPIO_PORT,USART2_TX_SOURCE,GPIO_AF_USART2); //GPIOA2复用为USART2
	GPIO_PinAFConfig(USART2_GPIO_PORT,USART2_RX_SOURCE,GPIO_AF_USART2); //GPIOA3复用为USART2
	
	//USART2    
    GPIO_InitStructure.GPIO_Pin = USART2_RX_PIN | USART2_TX_PIN; //GPIOA2与GPIOA3
