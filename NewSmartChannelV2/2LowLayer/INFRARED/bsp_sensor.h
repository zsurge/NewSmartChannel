...
#define TURN_ON	    0		//红外触发
#define TURN_OFF	1		//无触发
...
{	
    uint8_t state[SENSOR_NUM_MAX];		        //上一次扫描时红外的值
	uint8_t curSensorVal[SENSOR_NUM_MAX];		//当前红外的值
	uint32_t code;			        //当前所有红外状态的值
	uint32_t preCode;		        //上一次红外的值	          
}SENSORVALUE_STRU;
